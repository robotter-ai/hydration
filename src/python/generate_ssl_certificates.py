...
@click.command()
@click.option("-p", "--password", prompt="1", help="The password in which your private key will be encripted", type=str, required=True)
@click.option("-d", "--destination", prompt="2", help="The path in which your certificates will be placed", type=str, required=True)
def create_self_sign_certs(password: str, destination: str):
    """
    Create self-sign CA Cert
    """

    filepath_list = {
        'ca_key': join(destination, ca_key_filename),
        'ca_cert': join(destination, ca_cert_filename),
        'server_key': join(destination, server_key_filename),
        'server_cert': join(destination, server_cert_filename),
        'server_csr': join(destination, server_csr_filename),
        'client_key': join(destination, client_key_filename),
        'client_cert': join(destination, client_cert_filename),
        'client_csr': join(destination, client_csr_filename)
    }

    # Create CA Private & Public Keys for signing
    ca_private_key = generate_private_key(password, filepath_list['ca_key'])
    generate_public_key(ca_private_key, filepath_list['ca_cert'])

    # Create Server Private & Public Keys for signing
    server_private_key = generate_private_key(password, filepath_list['server_key'])
    # Create CSR
